...
const Billing = require('../models/Erika_quotation_model');  // Assuming you have a Billing model
...
exports.createQuotation = async (req, res) => {
  try {
    ...
    const updatedProducts = products.map((product) => {
      ...
      const gst = product.gst || 0; // Default GST to 18% if not provided
    ...
    const htmlContent = await ejs.renderFile(path.join(__dirname, '../views/erika_quotation.ejs'), {
      client,
    ...
    const pdfPath = path.join(__dirname, '../Erika_quotation', `quotation_${newBilling._id}.pdf`);
    ...
    const options = {
      ...
      type: 'pdf',
      quality: '100',
      renderDelay: 1000, // Allow some time for rendering
    };
    ...
    pdf.create(htmlContent, options).toFile(pdfPath, (err, result) => {
      ...
      res.status(200).json({
        ...
        pdfPath: `/Erika_quotation/quotation_${newBilling._id}.pdf`,
      });
    });
